...
# NEED TO MODIFY SO THAT THE SORTED ARRAY THAT IS RETURNED HAS THE VALUES OF THE ORIGINAL ARRAY,
# BEFORE THE TRANSFORMATION ON THE BASIS OF WHICH IT IS SORTED.

def bubble_sort2!(array)
  copy = array.dup
  array.map! { |element| yield(element) } if block_given?
  conversion = array.zip(copy).to_h
  loop do
    swapped = false
  ...
  array.map! { |item| conversion[item] }
  nil
end
